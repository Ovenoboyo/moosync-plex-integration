...
export class MyExtension implements MoosyncExtensionTemplate {
                ...
                for (const a of artists) {
                    ...
                    for (const al of albums) {
                        ...
                        for (const s of songs.MediaContainer.Metadata) {
                            ...
                            if (parsed) {
                                const existing = await api.getSongs({ song: { _id: parsed._id } })
    ...
    private async getAllArtists(key: string) {
        const resp = await this.axios.get<AllArtists>(`${this.baseURL}/library/sections/${key}/all?X-Plex-Token=${this.token}`)
